...
export const ScholarLiveStream: React.FC = () => {
  ...
  // currentStreamData removed - using streamId directly
  ...
  const startStream = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Unable to reach internet. Livestreaming requires stable internet connection. Please check your network and try again.')
      }
      ...
      try {
        ...
        if (joinError.code === 'INVALID_OPERATION') {
          throw new Error('Streaming service error. Please check your internet connection and try again.')
        }
        
        if (joinError.code === 'CAN_NOT_GET_GATEWAY_SERVER' || joinError.message?.includes('CAN_NOT_GET_GATEWAY_SERVER') || joinError.message?.includes('network')) {
          throw new Error('Cannot reach streaming servers. Please check:\n• Internet connection is stable\n• Firewall allows streaming\n• No VPN blocking connection\n\nTry moving to area with better signal.')
        }
        
        if (joinError.code === 'NETWORK_ERROR' || joinError.message?.includes('NetworkError') || joinError.message?.includes('Failed to fetch')) {
...
Why this works: Switches from token mode to simpler App ID mode for testing.`)
      ...
      if (streamError) {
          ...
          .on(
            ...
            async (payload) => {
              ...
              if (data) {
                ...
                console.log('✅ Adding comment to scholar view:', newComment)
        ...
        if (existingComments) {
          const formattedComments = existingComments.map((comment: any) => ({
            id: comment.id,
          ...
          console.log(`✅ Loaded ${formattedComments.length} existing comments`)
        }
      }
        ...
        <CardContent className="space-y-6">
          ...
          {!isStreaming ? (
            ...
            <div className="space-y-4">
              ...
              {/* Live Chat Display (Read-only for Scholar) */}
              <div className="border rounded-lg p-3 space-y-2 bg-gray-50">
                <h3 className="text-sm font-semibold mb-2 flex items-center gap-2">
                  💬 Live Chat
                  <span className="text-xs text-muted-foreground font-normal">({comments.length} messages)</span>
                </h3>
                
                {/* Comments Display */}
                <div className="h-40 overflow-y-auto space-y-2 mb-2 bg-white rounded p-2 border">
                  {comments.length === 0 ? (
                    <p className="text-xs text-gray-400 text-center py-4">No messages yet. Viewers can send messages during the stream.</p>
                  ) : (
                ...
                <p className="text-xs text-muted-foreground italic text-center">
                  You can see viewer messages but cannot reply during the stream
                </p>
              </div>
