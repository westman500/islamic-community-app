  ...
  const joinStream = async (stream: ActiveStream, bypassPayment = false) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to join stream:', joinError)
        
        // Check for network-related errors
        if (joinError.code === 'CAN_NOT_GET_GATEWAY_SERVER' || 
            joinError.code === 'NETWORK_ERROR' || 
            joinError.message?.includes('network') ||
            joinError.message?.includes('Failed to fetch')) {
          throw new Error('Cannot connect to streaming server. Please check your internet connection and try again.')
        }
        
        if (joinError.code === 'INVALID_OPERATION') {
          throw new Error('Streaming connection failed. Please check your internet and try again.')
        }
        
        throw new Error(`Failed to join stream: ${joinError.message || 'Unknown error'}`)
      }
